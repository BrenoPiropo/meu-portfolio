...
function AnimatedCounter({ target, suffix = "" }: { target: number; suffix?: string }) {
  ...
  return (
    <span ref={ref} className="bg-gradient-to-b from-white to-blue-400 bg-clip-text text-transparent">
      {count}{suffix}
    </span>
  );
}
...
export default function ProjetoPage() {
  ...
  // Extract metrics from resultado text
  const metrics = projeto.resultado.match(/\d+[\+%]?/g)?.slice(0, 3) || [];

  return (
    <main className="min-h-screen bg-[#050505] text-white selection:bg-blue-500/30 antialiased overflow-x-hidden">
      <CursorGlow />
      ...
      <article className="relative z-10 max-w-4xl mx-auto px-6 pt-32 pb-28">
        ...
        <AnimatedSection animation="scale-in" delay={300}>
          <div
            className={`aspect-video bg-[#0a0a0a] rounded-3xl border ${projeto.borderColor} overflow-hidden mb-20 relative group cursor-pointer transition-all duration-500 hover:shadow-lg`}
